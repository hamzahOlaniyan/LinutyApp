...
export async function updatePost(postId: string, value: any) {
   const { error } = await supabase.from("posts").update(value).eq("id", postId).throwOnError();

   return { error };
}

// export async function getUserPostsWithMedia(userId: string) {
//    const { data, error } = await supabase.from("posts").select("*").eq("author", userId);
//    // .throwOnError();
//    // .not("media", "is", null) // only posts with non-null media
//    // .neq("media", "[]") // exclude empty arrays
//    // .order("created_at", { ascending: false });
